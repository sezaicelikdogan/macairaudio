...
static int cs4208_probe(struct hda_codec *codec)
{
	...
	//mb9-specific
	err = setup_a1534(codec);          // keep: exposes the 4-channel PCM on hw:PCH,0
	//err = play_a1534(codec);         // DISABLED (MacBook8,1): interferes on kernel 7.0
	hda_codec_to_driver(codec)->ops = &cs_4208_patch_ops_explicit;
	//spec->gen.pcm_playback_hook = cs_4208_playback_pcm_hook;  // DISABLED: per-open hook reroutes to a dead tag; the mb81-speakers daemon controls the codec instead
	//end
